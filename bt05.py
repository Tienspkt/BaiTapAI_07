...
def ChuyenDoiAnhMauRBGSangAnhXamLuminance(imgPIL):

    #tạo 1 ảnh có cùng kích thước  và mode với ảnh imgPIL
    #ẢNH này chứa các kết quả chuyển đồi RGB sáng Grayscale
    Anh_Luminance= Image.new(imgPIL.mode, imgPIL.size)

    #lấy kích thước anh từ imgPIL
    width= Anh_Luminance.size[0]   #tiên biến width và size 0 là chiều ngang
    height= Anh_Luminance.size[1]   #tiên biến height và size 1 là chiều cao
    ...
    for x in range(width):
        for y in range(height):
            ...
            Anh_Luminance.putpixel((x, y), (gray, gray, gray)) #ảnh gồm 3 kênh mình cho cùng giá trị gray
    return Anh_Luminance
...
def TinhHistogram(HinhXamPIL):
    ...
    for x in range(w):
        for y in range(h):
            ...
            gR,gB,gG = HinhXamPIL.getpixel((x,y))
            #giá trị gray tính ra cũng chính là phần tử thứ gray
            #trong mảng his ddax khai báo ở trên, sễ tắng số đếm của các phần tử thứ gray lên 1
            his[gR]+=1
    return his
